...
When adding or renaming a command, update the shell completion scripts in `src/commands/init.rs` (bash, zsh, and fish) to include the new command name. The completion lists are hardcoded strings, not generated from `cli.rs`, so they must be kept in sync manually. The shell wrapper's case list (`add|switch|clone|remove|rm|clean`) must also be updated if a new command prints a path to stdout for cd purposes.
